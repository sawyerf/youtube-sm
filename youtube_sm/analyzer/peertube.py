...
class Peertube_Analyzer(Analyzer):
	...
	def add_sub(self, url):
		...
		try:
			...
			self.Error("Wrong host ({})".format(sub), 1)
			return None
		if data is None or '</rss>' not in data:
			self.Error("This sub can't be add.", 1)
			return None
